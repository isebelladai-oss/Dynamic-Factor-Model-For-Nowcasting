...
| `02_rolling_window_nowcasting/滚动窗口模型效果汇总.ipynb` | 汇总滚动窗口训练期 filter 拟合、smoother 重构与样本外 nowcast |
...
项目的整体流程可概括为“数据准备 -> 数据预处理 -> 动态因子模型 -> 参数估计 -> 实时预测 -> 市场检验”，如下图所示：
...
项目采用**固定十年训练窗口、逐年滚动更新**的设定：每次使用连续 10 年样本重新估计模型参数，并据此预测下一年的 GDP。

- 第一个训练窗口为 `2005-2014`
- 此后按年向前滚动，并在每个新窗口下重新估计参数
- 当前仓库覆盖的最新窗口为 `2016-2025`

这样做的目的，是在保证训练样本长度的同时，使参数估计尽可能反映更接近当期制度环境与经济结构的关系。
...
| `All` | 1266 | -0.0091 | 0.3828 | 0.5245 | 0.9680 |
...
| `All` | 1266 | -0.0245 | 0.2059 | 0.2909 | 0.9902 |

### 11.2 样本外 nowcasting 结果
...
- 核心公式为 $IC = \operatorname{Corr}(GDP_t, r_{t+1})$
- 进一步考察排序意义上的单调关系，计算 $RankIC = \operatorname{Corr}(\operatorname{Rank}(GDP_t), \operatorname{Rank}(r_{t+1}))$
...
回归形式可写为：$bond\_ret_{t+1} = \alpha + \beta \cdot Nowcasting\ GDP_t + \gamma' controls_t + \varepsilon_{t+1}$，其中 $controls_t$ 包括宏观经济变量、流动性变量等控制项。
...
### 13.1 DFM + 状态空间框架适合做 GDP nowcasting
...
### 13.3 nowcasting 信号对债券有解释力，但稳定性有限
